...
/**
 * GameCard
 * @param {{ game: any, heroUsername: string, onSelect: (game: any) => void }}
 */
export default function GameCard({ game: g, heroUsername, onClick }) {
  const hero = heroUsername.trim().toLowerCase();
  const whiteU = g.white.username.toLowerCase();
  const blackU = g.black.username.toLowerCase();
  const heroSide = whiteU === hero ? 'white' : blackU === hero ? 'black' : null;
  ...
  const openingName = rawName.split('.')[0].replace(/-/g, ' ');
    ...
    >
      ...
      <div className="mb-2">
        <h3 className="text-lg font-semibold text-white">
          {g.white.username}{' '}
          <span className="text-sm text-gray-400">({g.white.rating})</span>
          <span className="mx-2 text-gray-500">vs</span>
          {g.black.username}{' '}
          <span className="text-sm text-gray-400">({g.black.rating})</span>
        </h3>
      </div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-4 text-sm text-gray-400">
        ...
        <div className="flex items-center space-x-1">
          ...
          <span>
            {humanTC}{' '}
            <span className="italic">({g.rated ? 'Rated' : 'Casual'})</span>
          </span>
        </div>
        {g.termination && (
          <div>
