...
import { DOT_COLOR, INACCURACY, TIME_TEXT_COLOR } from '@/lib/severity';
...
function LegendDot({
  ...
  const formatLabel = (label: string, count: number): string => {
    const noPlural = ['time control'];
    if (count === 1 || noPlural.includes(label)) return label;
  ...
  return (
    <>
      {entries.map((r, i, all) => {
        ...
        const timeColor = TIME_TEXT_COLOR[r.severity];
          ...
          >
            <div className="grid grid-cols-3 items-center">
              ...
              <div className="text-right">
                <span
                  className={`-mr-1 inline-block rounded-full px-3 py-1 text-xs font-semibold ${DOT_COLOR[r.severity]} text-white`}
                >
          ...
          return (
            ...
            >
              <td className="px-2 py-1">
                <span
                  className={`inline-block h-4 w-4 rounded-full ${DOT_COLOR[r.severity]}`}
                />
              </td>
              ...
              <td className={`px-2 py-1 text-right font-medium ${timeColor}`}>
    ...
    <>
      ...
      <div className="mb-2 flex flex-wrap items-center space-x-4 px-2 text-xs text-gray-500">
        {severityOrder.map((sev) => (
          <LegendDot
            key={sev}
            colour={dotColour[sev]} // now comes from your config map
            count={counts[sev]}
            label={sev}
          />
        ))}
      ...
      {viewMode === 'card' ? (
        <CardView entries={combined} showAll={showAll} onDrill={onDrill} />
      ) : (
        <TableView entries={combined} showAll={showAll} />
      )}
    </>
  );
}
